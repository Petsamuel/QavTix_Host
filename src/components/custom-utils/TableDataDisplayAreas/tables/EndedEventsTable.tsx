...
function EndedCancelledTable({
    ...
    if (isEmpty || !items.length) {
        ...
        return (
            <div className="my-10">
                <EmptyTicketsState
                    title={isEnded ? "No Ended Events" : "No Cancelled Events"}
                    text={isEnded ? "You don't have any ended events" : "You don't have any cancelled events"}
                />
            </div>
        )
    }
                                    ...
                                    >
                                        ...
                                        <td className="py-4 px-5">
                                            <div className="flex items-center gap-1 whitespace-nowrap">
                                                <Icon icon="mdi:circle" className={cn("w-2 h-2", badgeCls)} />
                                                <span className={cn("text-xs font-medium", badgeCls)}>{badgeText}</span>
                                            </div>
                                        </td>
                                        <td className="py-4 px-5">
                            ...
                            <div className="space-y-3">
                                <div className="flex items-center justify-between text-xs pb-2 border-b border-brand-neutral-2">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex items-center gap-1 whitespace-nowrap">
                                            <Icon icon="mdi:circle" className={cn("w-2 h-2", badgeCls)} />
                                            <span className={cn("text-[11px] font-medium", badgeCls)}>{badgeText}</span>
                                        </div>
                                    </div>
...
export function EndedEventsTable(props: Omit<EndedCancelledTableProps, "variant">) {
    return <EndedCancelledTable {...props} variant="ended" />
}

export function CancelledEventsTable(props: Omit<EndedCancelledTableProps, "variant">) {
    return <EndedCancelledTable {...props} variant="cancelled" />
}